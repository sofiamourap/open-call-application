...
router.get("/:id", function (req, res) {
  const { id } = req.params;
  db(
    `SELECT openCall.residency_name, openCall.description, openCall.status, gallery.name FROM openCall JOIN gallery ON openCall.gallery_id = gallery.id WHERE openCall.id="${id}";`
  )
    //
    .then((result) => {
      res.send(result.data);
